...
- Fixed cross-version CI by preserving pending Pi RPC deadlines, replacing a timing-dependent child-exit assertion, resolving npm when Node runs through `npx`, and testing Node.js 20 without Node.js 22-only TypeScript hooks.
...
- Pasted images no longer render inline in chat: framed `[Image N]` messages keep their textual marker while the full image data still reaches the model, and inline images are enabled on Windows Terminal by forcing the Kitty protocol where detection reported none.
...
- Copied selections now strip message frame borders and box-drawing decorations so only the actual text lands on the clipboard; successful copies show a solid green “✓ Copied” badge inside the chat viewport above the composer.
