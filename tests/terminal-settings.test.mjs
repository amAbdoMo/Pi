...
test("terminal font configuration preserves settings and creates only one backup", () => {
  ...
  assert.equal(configureTerminalSettings(settingsFile, "CaskaydiaMono NFM"), true);
  assert.equal(configureTerminalSettings(settingsFile, "CaskaydiaMono NFM"), false);
  ...
  assert.equal(configured.profiles.defaults.font.face, "CaskaydiaMono NFM");
