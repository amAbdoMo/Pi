...
export type VerificationPhase = "clean" | "pending" | "verifying" | "repairing" | "awaiting-user" | "resume-pending" | VerificationStatus;
...
export class VerificationTracker {
	...
	snapshot(): VerificationSnapshot {
		return {
			...
			blocker: this.state.blocker ? { ...this.state.blocker } : undefined,
		};
	}
	...
	resumeAfterBlocker(): boolean {
		if (this.state.phase !== "awaiting-user" && this.state.phase !== "blocked") return false;
		this.state.phase = "resume-pending";
		this.state.attempts = 0;
		this.state.browser = { ...EMPTY_BROWSER_EVIDENCE };
		this.state.lastSummary = undefined;
		this.state.issues = [];
	...
	recordBlocker(blocker: VerificationBlocker): boolean {
		if (!this.requiresVerification() || this.state.phase === "awaiting-user") return false;
		const summary = collapseWhitespace(blocker.summary).slice(0, 500);
		if (!summary || !VERIFICATION_BLOCKER_KINDS.includes(blocker.kind)) return false;
		this.state.phase = "awaiting-user";
		this.state.lastSummary = summary;
		this.state.issues = [summary];
		this.state.blockerSequence += 1;
		this.state.blocker = {
			kind: blocker.kind,
			summary,
			sourceTool: blocker.sourceTool ? collapseWhitespace(blocker.sourceTool).slice(0, 100) : undefined,
			sequence: this.state.blockerSequence,
		};
		this.state.exhaustionNotified = false;
		return true;
	}
	...
	isAwaitingUser(): boolean {
		return this.state.phase === "awaiting-user";
	}

	stopAtBlocker(): boolean {
		if (!this.state.blocker || (this.state.phase !== "awaiting-user" && this.state.phase !== "blocked")) return false;
		this.state.phase = "blocked";
		return true;
	}

	currentBlocker(): VerificationBlocker | undefined {
		return this.state.blocker ? { ...this.state.blocker } : undefined;
	}

	recordMutation(options: { ui?: boolean } = {}): void {
		if (!this.state.enabled) return;
		if (["clean", "passed", "blocked", "awaiting-user"].includes(this.state.phase)) this.state.attempts = 0;
		this.state.phase = "pending";
	...
	recordSuccessfulCheck(command: string): boolean {
		...
		if (this.state.phase === "failed" || this.state.phase === "repairing" || this.state.phase === "resume-pending") {
	...
	recordBrowserTool(toolName: string): boolean {
		...
		if (!["awaiting-user", "blocked"].includes(this.state.phase)) this.state.phase = "verifying";
...
Do not give a completion response until verification_report accepts status passed.`;
	...
	statusText(): string {
		...
		switch (this.state.phase) {
			...
			case "verifying": return "verification running";
			case "repairing": return `verification repair ${this.state.attempts}/${MAX_REPAIR_ATTEMPTS}`;
			case "awaiting-user": return "verification awaiting user";
			case "resume-pending": return "verification resuming";
...
function normalizeSnapshot(candidate: unknown): VerificationSnapshot {
	...
	const phase = ["clean", "pending", "verifying", "repairing", "awaiting-user", "resume-pending", "passed", "failed", "blocked"].includes(String(record.phase))
	...
	return {
		...
		blocker,
		blockerSequence: boundedInteger(record.blockerSequence, 0, Number.MAX_SAFE_INTEGER),
		exhaustionNotified: record.exhaustionNotified === true,
	};
}

function normalizeBlocker(candidate: unknown): VerificationBlocker | undefined {
	if (!isRecord(candidate) || !VERIFICATION_BLOCKER_KINDS.includes(candidate.kind as VerificationBlockerKind)) return undefined;
	const summary = typeof candidate.summary === "string" ? collapseWhitespace(candidate.summary).slice(0, 500) : "";
	if (!summary) return undefined;
	return {
		kind: candidate.kind as VerificationBlockerKind,
		summary,
		sourceTool: typeof candidate.sourceTool === "string" ? collapseWhitespace(candidate.sourceTool).slice(0, 100) : undefined,
		sequence: boundedInteger(candidate.sequence, 0, Number.MAX_SAFE_INTEGER),
	};
}
