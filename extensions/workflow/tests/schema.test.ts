...
describe("secure YAML compiler and discovery", () => {
	...
	test("never imports CWD-controlled Python modules during pre-trust global discovery", () => {
		...
		const result = discoverWorkflows(cwd, false, { builtInDir: path.join(root, "built-in"), globalDir, projectDir: path.join(cwd, ".pi", "workflows") });
	...
	test("rejects unsafe entries, oversized files, and duplicate suffix IDs promptly", () => {
		const root = tempRoot();
		...
		if (process.platform !== "win32") {
			...
			spawnSync("mkfifo", [path.join(globalDir, "pipe.yaml")]);
		}
		...
		const result = discoverWorkflows(root, false, { builtInDir, globalDir });
		expect(Date.now() - started).toBeLessThan(1000);
		expect(result.workflows).toHaveLength(0);
		expect(result.diagnostics.map((item) => item.message).join("\n")).toMatch(/exceeds|Duplicate/);
		if (process.platform !== "win32") {
			expect(result.diagnostics.map((item) => item.message).join("\n")).toMatch(/non-symlink|regular/);
			const linkedDir = path.join(root, "linked-dir");
			fs.symlinkSync(globalDir, linkedDir, "dir");
			const directoryResult = discoverWorkflows(root, false, { builtInDir, globalDir: linkedDir });
	...
	test("discovers built-ins while allowing a global workflow to override the same ID", () => {
		const root = tempRoot();
		const builtInDir = path.join(root, "built-in");
		const globalDir = path.join(root, "global");
		write(builtInDir, "pipeline.yaml", minimal("built-in"));
		let result = discoverWorkflows(root, false, { builtInDir, globalDir });
		expect(result.workflows.map((item) => [item.id, item.source])).toEqual([["pipeline", "built-in"]]);

		write(globalDir, "pipeline.yaml", minimal("global"));
		result = discoverWorkflows(root, false, { builtInDir, globalDir });
		expect(result.workflows.map((item) => [item.id, item.source])).toEqual([["pipeline", "global"]]);
	});
	...
	test("an invalid trusted project definition shadows a valid global ID", () => {
		...
		const result = discoverWorkflows(root, true, { builtInDir: path.join(root, "built-in"), globalDir, projectDir });
