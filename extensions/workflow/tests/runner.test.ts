...
describe("workflow state machine", () => {
	...
	test("propagates ephemeral sessions and orders normal append before phase append", async () => {
		const root = fs.mkdtempSync(path.join(os.tmpdir(), "workflow-runner-append-"));
		try {
			const configDir = path.join(root, ".pi");
			fs.mkdirSync(configDir);
			fs.writeFileSync(path.join(configDir, "APPEND_SYSTEM.md"), "project instructions");
			...
			expect(args[appendIndexes[0] + 1].endsWith("APPEND_SYSTEM.md")).toBe(true);
			...
			const progressNotes = h.sent.filter((message) => message.customType === "workflow-info" && typeof message.content === "string" && message.content.startsWith("▶"));
