...
const fixture = path.join(path.dirname(fileURLToPath(import.meta.url)), "fixtures", "fake-rpc-child.mjs");
...
describe("bounded RPC lifecycle", () => {
	...
	test("uses taskkill-style tree termination on Windows", async () => {
		const fake: any = new EventEmitter();
		...
		fake.signalCode = null;
		fake.pid = 4242;
		fake.kill = () => { throw new Error("direct leader kill must not be used on Windows"); };
		let killedPid = 0;
	...
	test("terminates an active RPC child tree on the current platform", async () => {
		...
		const rpc = client("active-descendant", { termGraceMs: 200, killGraceMs: 1000 }, { WF_RPC_DESCENDANT_PID_FILE: pidFile });
		let descendantPid = 0;
		try {
			...
			await rpc.stop();
			for (let attempt = 0; attempt < 100 && processExists(descendantPid); attempt++) await new Promise((resolve) => setTimeout(resolve, 10));
