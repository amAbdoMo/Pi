...
export type KillWindowsProcessTree = (pid: number, timeoutMs: number) => Promise<void>;

export interface RpcProcessControl {
	platform?: NodeJS.Platform;
	killWindowsProcessTree?: KillWindowsProcessTree;
}

export function terminateWindowsProcessTree(pid: number, timeoutMs: number): Promise<void> {
...
export class RpcPhaseClient implements RpcPhaseTransport {
	...
	constructor(
		...
		processControl: RpcProcessControl = {},
	) {
		this.bounds = { ...DEFAULT_RPC_BOUNDS, ...bounds };
		this.platform = processControl.platform ?? process.platform;
		this.killWindowsProcessTree = processControl.killWindowsProcessTree ?? terminateWindowsProcessTree;
	...
	stop(): Promise<void> {
		...
		this.stopPromise = (async () => {
			...
			if (this.platform === "win32" && Number.isSafeInteger(this.proc.pid) && (this.proc.pid ?? 0) > 0) {
				try {
				...
				const error = new Error(`RPC child process tree did not exit after taskkill. Stderr: ${this.getStderr()}`);
				this.fail(error);
				throw error;
			}
			this.signal("SIGTERM");
