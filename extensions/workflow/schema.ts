...
export function discoverWorkflows(cwd: string, projectTrusted: boolean, directories: { builtInDir?: string; globalDir?: string; projectDir?: string; beforeRead?: (filePath: string) => void } = {}): WorkflowDiscovery {
	const diagnostics: WorkflowDiagnostic[] = [];
	...
	loadScope(directories.builtInDir ?? getBuiltInWorkflowDir(), "built-in", false);
	loadScope(directories.globalDir ?? getGlobalWorkflowDir(), "global", true);
...
export function workflowListMarkdown(discovery: WorkflowDiscovery): string {
	const lines = ["# Workflows", "", "Pi Workbench includes `pipeline`; add lowercase YAML files in `~/.pi/workflows` or a trusted project's `.pi/workflows` directory to override or extend it.", ""];
